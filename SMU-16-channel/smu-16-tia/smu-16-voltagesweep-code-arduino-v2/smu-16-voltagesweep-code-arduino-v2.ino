...
float drain_voltage_supply = 0.11; // (in volts)
float offset_voltage_tia = 1.5; // 1.5, positive value
float gate_start_voltage = -1; // minimum value is -1.5 [-1 * offset_voltage_tia]
float gate_end_voltage = 1; // maximum value is 1.8 [-1 * offset_voltage_tia + 3.3]
float R_f = 10000; // negative feedback resistor for transimpedance aplifier
float start_time_s;
...
void setup() {
  ...
  uint16_t drain_digital_value = (uint16_t)((offset_voltage_tia + drain_voltage_supply) / 3.3 * 4095);
  dac_drain.setVoltage(drain_digital_value, false);


  // Set start gate voltage
  uint16_t gate_digital_value = (uint16_t)((offset_voltage_tia + gate_start_voltage) / 3.3 * 4095);
